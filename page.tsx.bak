...
export default function Home() {
  ...
  const fetchKegiatanPublik = async () => {
    try {
      ...
      if (error) {
        setKegiatanError(`Gagal memuat kegiatan: ${error.message}`);
        setKegiatanData([]);
        return;
      }
  ...
  return (
    <main className="w-full h-full items-center justify-center">
      ...
      <section className="py-5 bg-gradient-to-r from-gray-100 to-gray-200">
        ...
        <div className="container mx-auto px-6">
          <div className="flex justify-center items-center overflow-hidden bg-white rounded-xl shadow-lg p-4">
            <motion.div
              ...
              >
                {testimoniData.map((testimoni, index) => {
                  ...
                  return (
                    <motion.div
                      key={testimoni.id}
                      className="w-[90%] md:w-[60%] lg:w-[45%] flex-shrink-0 p-3 md:p-4 transition-opacity duration-300"
                      animate={{
                        scale: index === currentTestimonialIndex ? 1.0 : 0.9,
                      ...
                      <div className="bg-white rounded-xl shadow-lg pt-4 px-4 pb-3 flex flex-col transition-all duration-300 h-full">
                        <div className="flex flex-col sm:flex-row items-start mb-3 w-full">
                          ...
                          <div className="sm:ml-4 mt-2 sm:mt-0 flex flex-col flex-grow">
                            ...
                            <div className="w-full h-[1px] bg-gray-200 my-1.5"></div>
                            <p className="text-xs md:text-sm text-gray-700 font-semibold mb-1.5">
                              {testimoni.prodi} - {testimoni.angkatan}
                            </p>
                            <p className="text-xs md:text-sm text-gray-600 italic leading-snug mt-1 mb-0">
                              "<span className="inline">{testimoniText}</span>"
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              {testimoniData.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/60 hover:bg-white/90 backdrop-blur-sm p-2 rounded-full shadow-lg transition-all z-20"
                  ...
                  <button
                    onClick={() => paginateTestimonials(1)}
                    className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/60 hover:bg-white/90 backdrop-blur-sm p-2 rounded-full shadow-lg transition-all z-20"
...
const AnimatedCounter = ({
  ...
  useEffect(() => {
    ...
    let startValue = 0;
    let endValue = value;
    let duration = value > 1000 ? 2500 : 2000;
